...
##Summary- 
This assignment is about using the Naive Bayes algorithm to predict whether a customer will accept a personal loan offer. We’ll be working with the UniversalBank.csv dataset, which contains information on 5,000 customers—their demographics, banking habits, and how they responded to a previous loan campaign. Interestingly, only 9.6% of them actually took the loan. Our focus will be on two main factors: whether the customer uses online banking and whether they have a credit card from the bank. Using these, we’ll try to predict the Personal Loan outcome. The data will be split into 60% for training and 40% for validation to test our model’s performance.
...
Unibank_data <- read.csv("C:\\Users\\pedap\\Documents\\FML\\UP Assignment 3\\UniversalBank (2).csv")
head(Unibank_data,3)
```
...
train_set.m = reshape2::melt(training.df,id=c("CreditCard","PersonalLoan"),variable= "Online")
train_set.d = reshape2::dcast(train_set.m,CreditCard+PersonalLoan~Online)
train_set.d
head(train_set.m,3)
